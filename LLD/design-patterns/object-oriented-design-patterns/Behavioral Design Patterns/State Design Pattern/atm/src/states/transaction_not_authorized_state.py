...
class TransactionNotAuthorizedState(TransactionState):

    @override
    def add_card_to_transaction_context(self, transaction_context: 'TransactionContext', card: Card) -> None:
        raise IllegalTransitionOnStateException(expected_state=TransactionNotAuthorizedState(), actual_state=transaction_context.get_transaction_state())
    ...
    def reset_transaction_context(self, transaction_context: 'TransactionContext'):
        if type(transaction_context.get_transaction_state()) is not TransactionNotAuthorizedState:
            raise IllegalTransitionOnStateException(expected_state=TransactionNotAuthorizedState(), actual_state=transaction_context.get_transaction_state())
    ...
    def __repr__(self) -> str:
        return f'TransactionNotAuthorizedState'
